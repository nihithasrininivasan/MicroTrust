...
import { ChevronLeft, Ticket, Zap, Gift, ShoppingBag, ArrowRight } from 'lucide-react';
...
const RewardsPage: React.FC<Props> = ({ user, lang, onBack }) => {
  ...
  const rewards = [
    { title: 'Electricity Cashback', provider: 'BESCOM', value: '₹50 OFF', icon: <Zap size={20}/>, scoreNeeded: 500 },
    { title: 'Merchant Perk', provider: 'Amazon Pay', value: '5% Boost', icon: <ShoppingBag size={20}/>, scoreNeeded: 650 },
    { title: 'Credit Multiplier', provider: 'Microtrust', value: '2x Bonus', icon: <Gift size={20}/>, scoreNeeded: 750 },
  ];
    ...
    <div className="space-y-8 pb-24 animate-in slide-in-from-right-8 duration-500">
      ...
      <div className="space-y-4">
         ...
         {rewards.map((r, i) => {
            ...
            return (
               <div key={i} className={`nm-flat rounded-3xl p-5 flex items-center justify-between transition-all ${isLocked ? 'opacity-50 grayscale' : 'hover:scale-[1.02]'}`}>
