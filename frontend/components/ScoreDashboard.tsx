...
import { getScoreExplanation } from '../services/geminiService';
...
const ScoreDashboard: React.FC<Props> = ({ user, lang }) => {
  ...
  const fetchAdvice = async () => {
    ...
    const result = await getScoreExplanation(user, lang);
    setAdvice(result);
      ...
      <section className="nm-flat rounded-3xl p-7 space-y-5">
        ...
        {loadingAdvice ? (
          <div className="flex items-center gap-4 py-4">
            ...
            <span className="text-xs text-[#6B6B6B] font-medium tracking-wide">Synthesizing insights...</span>
          </div>
        ) : (
