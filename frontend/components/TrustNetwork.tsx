...
import { Users, UserCheck, ShieldPlus, MessageSquare, AlertTriangle, X, Shield, Phone, User as UserIcon, CheckCircle2, Briefcase, ArrowUpRight, BarChart3, Info, Star } from 'lucide-react';

interface Props {
  lang: Language;
}

const TrustNetwork: React.FC<Props> = ({ lang }) => {
  ...
  const [endorsements, setEndorsements] = useState<Endorsement[]>([
    { id: '1', name: 'Ramesh Kirana Store', score: 820, status: 'Verified', type: 'Merchant', strength: 5, duration: 24 },
    { id: '2', name: 'Sunita (Vendor)', score: 710, status: 'Verified', type: 'Peer', strength: 4, duration: 12 },
    { id: '3', name: 'Alok Kumar', score: 650, status: 'Pending', type: 'Peer', strength: 3, duration: 6 },
  ]);

  const [newRef, setNewRef] = useState<CommunityReference>({ name: '', role: 'Supplier', phone: '', strength: 3, duration: '1-6 Months' });

  const communityStrength = Math.round((endorsements.filter(e => e.status === 'Verified').length / 5) * 100);
        ...
        <div className="fixed inset-0 z-[120] bg-black/40 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in slide-in-from-bottom-10 duration-300">
          <div className="nm-flat rounded-t-[40px] sm:rounded-[40px] p-8 space-y-8 w-full max-w-md relative shadow-2xl">
            ...
            <div className="nm-flat-sm p-6 rounded-[32px] space-y-4">
               ...
               <div className="space-y-3">
                 ...
                 <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-bold text-[#1C1C1C]">Merchant Network</span>
                 </div>
               </div>
            </div>
            ...
            <button 
              ...
              className="w-full nm-orange text-white py-4 rounded-full font-bold uppercase tracking-widest text-[10px] shadow-lg"
            >
              Back to Network
            </button>
          </div>
          ...
          <div className="nm-flat rounded-[40px] p-8 space-y-6 w-full max-w-sm max-h-[85vh] overflow-y-auto relative shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4 pt-2">
                {[
                  { title: "1. Multi-Signal Verification", desc: "One reference has small weight. We collective analyze consistency across 3+ verified contacts to detect real reputation." },
      ...
      {showAddForm && (
        <div className="fixed inset-0 z-[110] bg-black/40 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in zoom-in-95">
          <form onSubmit={handleAddReference} className="nm-flat rounded-[40px] p-8 space-y-6 w-full max-w-sm relative shadow-2xl">
            ...
            <button type="submit" className="w-full nm-orange text-white py-4 rounded-full font-bold uppercase tracking-widest text-[10px] shadow-lg">Generate Invite OTP</button>
          </form>
        </div>
      )}
