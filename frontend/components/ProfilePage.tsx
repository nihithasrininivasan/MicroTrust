...
import { User, Mail, MapPin, Briefcase, Calendar, ChevronRight, LogOut, Settings, ShieldPlus, CheckCircle2, ShieldCheck, X, Info } from 'lucide-react';
...
const ProfilePage: React.FC<Props> = ({ user, lang }) => {
  ...
  return (
    <div className="space-y-8 pb-32 animate-in fade-in duration-500">
          ...
          <div className="nm-flat rounded-[40px] p-8 space-y-6 w-full max-w-sm relative shadow-2xl">
            ...
            {kycStep === 2 && (
                <div className="py-8 flex flex-col items-center gap-4">
                    ...
                    <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-widest">Mock API Confirmation...</p>
                </div>
            )}
      ...
      <section className="nm-flat rounded-[32px] p-8 space-y-6">
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-[#6B6B6B] uppercase tracking-tighter">{t.age}</p>
              <p className="text-sm font-bold text-[#1C1C1C]">{user.age || '28'} Years</p>
            </div>
          </div>
      ...
      {/* Transparency Policy Section - MOVED FROM TRUST NETWORK */}
      ...
      <section className="space-y-4">
        ...
        <button className="w-full nm-flat-sm text-[#D66A5E] py-4 rounded-full font-bold flex items-center justify-center gap-3 text-xs uppercase tracking-widest nm-pressed transition-nm mt-4">
          <LogOut size={16} />
          Sign Out
        </button>
      </section>
