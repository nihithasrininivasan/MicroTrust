...
import { ShieldCheck, Eye, Trash2, Clock, CheckCircle2, Info, Lock, EyeOff, Loader2 } from 'lucide-react';
...
const PrivacyDashboard: React.FC<Props> = ({ lang, onLogout }) => {
    ...
    <div className="space-y-8 pb-24 animate-in fade-in duration-500">
      ...
      <section className="space-y-4">
        ...
        <div className="nm-flat rounded-[32px] p-8 space-y-6 border border-[#D66A5E]/10">
          ...
          {deleteError && (
            <p className="text-xs text-red-500 font-medium text-center bg-red-50 py-2 rounded-xl">{deleteError}</p>
          )}
          ...
          {!showDeletionConfirm ? (
            ...
            <div className="space-y-4 p-6 nm-inset rounded-3xl animate-in zoom-in-95 duration-300">
              ...
              <div className="flex gap-4">
                <button onClick={() => setShowDeletionConfirm(false)} disabled={deleting} className="flex-1 nm-flat-sm text-[#6B6B6B] font-bold text-[10px] uppercase tracking-widest py-3 rounded-2xl">Back</button>
                <button
                  onClick={handleDeleteAccount}
                  disabled={deleting}
                  className="flex-1 bg-[#D66A5E] text-white font-bold text-[10px] uppercase tracking-widest py-3 rounded-2xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  {deleting ? <><Loader2 size={14} className="animate-spin" /> Deleting...</> : 'Erase All'}
                </button>
              </div>
            </div>
          )}
